...
class ErdosGameSmallKEnv(gym.Env):
    ...
    def __init__(self, K, potential, unif_prob, 
                ...
                geo_ps=[0.45, 0.5, 0.6, 0.7, 0.8], hash_states=None, smallK=None):
        ...
        self.smallK = smallK
        # add in extra "shadow" observation space when we have a smallK
        if smallK:
            sample_dict["K"] = smallK
            sample_dict["unif_high"] = max(3, smallK-3)
            sample_dict["geo_high"] = smallK - 1
            sample_dict["weights"] = self.weights[-smallK-1:]
            self.shadow_observation_space = spaces.ErdosState(**sample_dict)
    ...
    def _reset(self):
        ...
        if not self.smallK:
            ...
            obstate = self.observation_space.sample().astype("int")
            shadowstate = self.shadow_observation_space.sample().astype("int")
            self.game_state = np.zeros(obstate.shape)
            self.game_state[-len(shadowstate):] = shadowstate
        self.done = False
        self.reward = 0
        self.steps = 0
